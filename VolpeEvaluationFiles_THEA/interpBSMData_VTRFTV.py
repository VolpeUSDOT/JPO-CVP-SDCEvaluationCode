...
eventTableName = 'thea_warningvtrftv_core'
eventTimestampString = "metadataloggeneratedat > '2019-03-01 00:00:00' and metadataloggeneratedat < '2020-03-16 00:00:00'"
...
eventRecords.drop(columns=['vtrftvid', 'level_0', 'index'], inplace=True)
...
for index, event in eventRecords.iterrows():
    ...
    for interpColumn in BSMInterpColumns_Units:
        ...
        interpedHost[interpColumn[0]] = pandas.Series([point*interpColumn[1] if not interpColumn[2](point) else None for point in deciHostData])
        interpedRemote[interpColumn[0]] = pandas.Series([point*interpColumn[1] if not interpColumn[2](point) else None  for point in deciRemoteData])
...
#    totalGapLen = pandas.to_timedelta(0, 's')
#    numgaps = 0
#    eventTimestamps = [time for time in remoteVehicleBSMs['bsmTime']]
#    gaps = []
#    for i in range(0, len(eventTimestamps)-1):
#        timeDiff = eventTimestamps[i+1] - eventTimestamps[i]
#        if timeDiff >= pandas.to_timedelta(.2, 's'):
#            numgaps += 1
#            totalGapLen = totalGapLen + timeDiff
#            gaps.append(timeDiff)
#    
#    averageGapLength = totalGapLen/numgaps
#    print('median Gap Length: {}'.format(numpy.median(gaps)))
#    print('Nubmer of Gaps: {}'.format(numgaps))
#    print('Gaps: {}'.format(gaps))
    
eventRecords.to_sql(name='Volpe_warningVTRFTV_unique', con=MSSQLEngine, if_exists='append')   
    
#    plt.figure(eventnum)
#    eventnum+=1
#    columnToPlot = 'coredatalat'
#    plt.plot(remoteVehicleBSMs['bsmTime'], remoteVehicleBSMs[columnToPlot]*0.0000001, 'o')
#    plt.plot(interpedRemote['datetimestamp'], interpedRemote[columnToPlot], '.')

#columnToPlot = 'coredatalat'
#plt.plot(hostVehicleBSMs['metadataloggeneratedat'], hostVehicleBSMs[columnToPlot], '.')
#plt.plot(interpedRemote['coredatalong'], interpedRemote[columnToPlot], '.')