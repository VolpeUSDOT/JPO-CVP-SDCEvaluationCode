...
eventTableName = 'thea_warningima_core'
...
eventRecords.drop(columns=['imaid', 'level_0', 'index'], inplace=True)
...
for index, event in eventRecords.iterrows():
    ...
    remoteVehicleBSMs = pandas.read_sql(remotevehicleBSMQuery, con=MSSQLEngine)
    remoteVehicleBSMs.drop_duplicates(subset=['metadataloggeneratedat','coredatasecmark','coredatalat','coredatalong', 'coredataid'], inplace=True)
...
eventRecords.to_sql(name='Volpe_warningIMA_unique', con=MSSQLEngine, if_exists='append') 
