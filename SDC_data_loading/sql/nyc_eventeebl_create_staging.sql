--------------------------------------------------------------
-- Create the NYC_EVENTEEBL_STAGING table 
--------------------------------------------------------------
DROP TABLE IF EXISTS nyc_eventeebl_staging;

CREATE EXTERNAL TABLE IF NOT EXISTS nyc_eventeebl_staging(
  eventheader struct<
...
LOCATION 
's3a://DOT-SDC-CVP-STAGING-S3-BUCKET-NAME/cv/nyc/eventEEBL/';
--'s3a://DOT-SDC-CVP-STAGING-S3-BUCKET-NAME/cv/nyc/eventEEBL/';

CREATE TABLE IF NOT EXISTS nyc_eventeebl(
  eventheader struct<
    locationsource:string,
    asdfirmwareversion:string,
    eventalertactive:boolean,
